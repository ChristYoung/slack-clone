    ...
    return <Card className='__SignInCard w-full h-full p-8'>
        ...
        <CardContent className='space-y-5 px-0 pb-0'>
            <form className='space-y-2.5'>
                <Input disabled={spinning} value={email} placeholder='Email' type='email' onChange={(e) => {setEmail(e.target.value)}} required />
                <Input disabled={spinning} value={password} placeholder='Password' type='password' onChange={(e) => {setPassword(e.target.value)}} required />
                <Button type='submit' className='w-full' size={'lg'} disabled={spinning}>Continue</Button>
            </form>
            <Separator />
            <div className='flex flex-col gap-y-2.5'>
                <Button variant='outline' className='w-full' size={'lg'} disabled={spinning} onClick={() => onProviderSignIn('github')}><FaGithub /> Continue with Github</Button>
